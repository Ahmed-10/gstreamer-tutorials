...
int main(int argc, char *argv[]) {
  ...
  data.video_convert = gst_element_factory_make("nvvideoconvert", "video-convert");

  /* Create the empty pipeline */
  data.pipeline = gst_pipeline_new ("test-pipeline");

  if (!data.pipeline || !data.source || !data.convert || !data.resample || !data.sink || !data.video_sink || !data.video_convert) {
  ...
  gboolean audio_stream_link = gst_element_link_many(data.convert, data.resample, data.sink, NULL);
  gboolean video_stream_link = gst_element_link_many(data.video_convert, data.video_sink, NULL);
  if (!audio_stream_link || !video_stream_link) {
...
static void pad_added_handler (GstElement *src, GstPad *new_pad, CustomData *data) {
  ...
  const gchar * structure = gst_structure_get_name(gst_caps_get_structure(new_pad_caps, 0));
  g_print("%s\n", structure);
  const gchar *new_pad_type = gst_structure_get_name(gst_caps_get_structure(new_pad_caps, 0));
  GstPad * sink_pad = NULL;
  
  /* get the suitable sink pad */
